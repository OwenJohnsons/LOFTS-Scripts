...
def ET_power_law(verbose=False):
    ...
    P = np.array([1e10,1e12,1e14,1e16,1e18,1e20,1e23,1e24, 1e26])

    #---------------------------
    # Luminosity function limit on putative transmitters - grey slope 
    plt.figure(figsize=(16, 9))
    # plt.plot(np.log10(P),np.log10(calc_NP_law3(P)),lw=20,color='gray',alpha=0.3)#,label=r'$\alpha$: %s'%alpha)

    plt.plot([17,17],[-13,6],'-',lw=5,color='black',alpha=0.5)#,label='Kardashev Type I')
    plt.plot([13,13],[-13,6], '-.', lw=5,color='black',alpha=0.5)#,label='AO Planetary Radar')
    #---------------------------
    ...
    print([np.log10(Price_BL_EIRP[2])],[np.log10(1./Price_BL_rarity[2])])
    ...
    plt.plot(16.9, -5.8, marker='*', markersize=markersize, color='lime', label='Johnson et al. (2023)')
...
def compare_SETI_limits(EIRP,rarity,shape='o',color='k',project='This Project',y_label_units=True):
    ...
    #EIRP=1e19 # Calculated EIRP
    
    ET_power_law()
    # - Plotting 2021 Galactic Center Survey - 
    plt.plot(17.69, -5.61,  marker='d', markersize=10, color='blue', label='Gajjar (2021 - GBT)')
    plt.plot(18.59, -8.09, marker = 'd',  markersize=10, color='orange', label='Gajjar (2021 - Parkes)')
    # - Plotting 
    plt.plot(18.21, -5.57, marker = '2', markersize=15, color='green', label='Suresh (2023)')
    ...
    plt.xlabel('EIRP $[\log(\\text{Watts})]$',fontsize = fontsize)
    #plt.ylabel('Transmiter Galactic Rarity [log((Nstars*BW)^-1)]',fontsize=fontsize)

    # if y_label_units:
    #     plt.ylabel('Transmitter Rate',fontsize=fontsize)
    #     #plt.ylabel(r"\Transmitter ~Rate ~log(\frac{1}{N_{stars}})",fontsize=fontsize)
    # else:
    plt.ylabel('Transmitter Rate $\left[ \log \left( \\frac{1}{N_{\\text{stars}} \cdot v_{\\text{rel}}} \\right) \\right]$ ',fontsize=fontsize)

    plt.xticks(fontsize = ticksize)
    plt.yticks(fontsize = ticksize)
    plt.ylim(-13,5)
    plt.xlim(10,25)
    plt.legend(loc=1, prop={'size': 12}, labelspacing = 1, frameon=True)
    #plt.savefig('SETI_limits_comparison.png', format='png',bbox_inches='tight')
    plt.savefig('./SETI_limits_comparison.pdf', format='pdf', dpi=300,bbox_inches='tight')
    # plt.show()
...
project = 'LOFAR'  #Project name
telescope = 'LOFAR'     # Telescope name
N_stars = 900000  # Estimated number of stars
band = 55e6     # Total bandwidth [Hz]
central_freq = 62e6 # Central bandwidth [Hz]

dish_diam = 22000  #Telescope diameter meters (single dish in current version)
dish_Tsys = 25  #Telescope Tsys [Kelvin]
dish_app_eff = 0.75  #Telescope Apperture Efficiency

SNR_threshold =  10 #Survey threshold   [sigma above the mean]
spectral_resolution = 1. #Spectral resolution [Hz]
scan_obs_time = 10*60 # Observation time per scan [sec]
max_distance = 6638.748825119999 #Distance + 1std  [Ly]
#max_distance = 200  #Maximum distance  [Ly]


iband = 40e6  #Instantaneous Bandwidth [Hz]
...
print('survey_rarity', np.log(1/survey_rarity))
